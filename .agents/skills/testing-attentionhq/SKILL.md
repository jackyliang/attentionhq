...
- `cd /home/ubuntu/repos/session-board && DEVIN_API_KEY="$DEVIN_API_KEY" GITHUB_TOKEN="$ATTENTIONHQ_GITHUB_TOKEN" OPENROUTER_API_KEY="$ATTENTIONHQ_OPENROUTER_API_KEY" BOARD_TOKEN=testtoken123 .venv/bin/uvicorn server:app --port 8420`
...
- Archive (X) persists to `dismissed.json` in the repo dir — delete it and restart the server to un-archive.
