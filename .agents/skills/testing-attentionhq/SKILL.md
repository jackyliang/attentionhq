...
- For session-start lifecycle tests, only use Ctrl+Enter when explicitly authorized to create a paid session or when `/api/prompt` is already intercepted. With authorized browser-only fixtures, delay the prompt response (`{ok:true,session_id:"qa-..."}`), then separately delay adding the matching raw `session:<id>` card to `/api/board` responses. Return a labeled test conversation from `/api/card/session%3A<id>/messages`. Keep other responses real and read-only; label the fixture on-screen and reload to remove it afterwards.
- Exercise both asynchronous boundaries: the response can assign a session ID while the card is still pending, then a later board refresh replaces it. Keep the panel open across both. Observe `#app`'s `data-mode` and thread text with a MutationObserver to catch brief panel closures or errors. Background thread prefetch may still request a pending key even when its visible thread correctly shows a starting note.
- With SSE connected, unchanged board versions may not fetch `/api/board` every five seconds: the fallback interval becomes 30 seconds. A fixture injected into board responses lands on the next actual board fetch, not necessarily at its eligibility time.
