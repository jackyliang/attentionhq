...
- ACP bridge (PR #97+): the server spawns `acp/bridge.mjs` when `DEVIN_ACP_API_KEY` (org secret, personal `cog_` PAT) is set, `node` >= 22 is on PATH (`source ~/.nvm/nvm.sh && nvm use 22` before starting uvicorn) and `acp/node_modules` exists (`npm ci --prefix acp`). Without any of those the log says `acp bridge disabled: ...` and everything falls back to the 5s REST poll. Pass `ACP_BRIDGE=0` to skip it. Check `curl -s -H 'x-board-token: testtoken123' localhost:8420/api/board | jq .sync.acp` (`enabled/connected/live/error`). Verified live 2026-09-11: `[acp] connected; ACP protocol 2`, ~24 sessions attached, this very session's replies stream in token by token (`ACP_DEBUG=1` logs every event). If `wss://api.devin.ai/acp/live` refuses the upgrade (it 403'd for everyone earlier that day), the bridge logs `connection lost: ...`, retries with backoff and the error shows in the sync popover row "Devin stream". Never stop the server with `pkill -f bridge.mjs` from a shell whose command line contains that string (kills the shell); `fuser -k 8420/tcp` is enough — an orphaned bridge exits on its own within ~5s.
- Bridge events can be faked without ACP: `POST /api/acp/events` `{"sessions":{"<session_id>":[{"type":"message","message_id":"m1","text":"streaming..."},{"type":"typing","typing":false}]}}` with the board token appends a Devin message to that session's thread (streaming ones show a blinking cursor) and pushes a `thread` SSE event; `/api/acp/hello` `{"connected":true}` flips the stream to live (REST reconcile slows to 60s) until 30s pass without another call.
...
- The "+" new-issue card is the FIRST card in the Issues column (focus a card, press Home); new issues appear directly under it (newest first). Enter opens inline input; Enter submits and creates a REAL GitHub issue (PR #96+: the server asks OpenRouter for `{repo,title,body}` — repo chosen from the board's tracked repos — and files it directly; Ctrl+Enter still starts a REAL Devin session — never press it in tests). Filing takes ~5-10s, so the in-flight placeholder (prompt as title + shimmer `.ref.ph-bar`, skeleton panel) is easy to miss — press Enter and screenshot in the same action batch. Prefix test prompts with `[attentionhq test — will be closed]` and close them afterwards (`PATCH .../issues/N {"state":"closed","state_reason":"not_planned"}`); the prompt text steers which repo the model picks (a "board Issues column" prompt → attentionhq, a "marketing site blog" prompt → answerhq-web).
- xdotool `type` drops em-dashes (`—`) — use `-` in typed prompts if the text must match exactly.
- Attaching a file to the "+" input or composer without a clipboard tool (no xclip on the box): build a File from base64 in the console and dispatch `new ClipboardEvent('paste',{clipboardData:dt,bubbles:true})` (with `dt=new DataTransfer(); dt.items.add(file)`) on `document.querySelector('#add-issue input')`; a chip appears in `.att-tray` and `POST /api/upload` 200 shows in the log. Issue bodies embed attachments as `![name](<base>/api/attachment/<uuid>/<name>?s=<32-hex hmac>)` — the `?s=` signature (HMAC of `uuid/name` with BOARD_TOKEN) is accepted without `x-board-token`; missing or wrong `s` → 401. `<base>` is `PUBLIC_URL`/`RENDER_EXTERNAL_URL` or the request base (http://localhost:8420 locally, so links in test issues are dead on GitHub itself).
...
- For live ACP UI proof, keep the target transcript at its bottom before an externally supplied reply starts. A short reply may finish in ~1s; a longer probe gives several seconds to capture the caret. Supplement the recording with a read-only MutationObserver on `#thread` to timestamp `.msg.streaming` states, and inspect a recorded frame rather than treating the DOM alone as visible proof. Do not send a probe from the card composer.
- To prove REST fallback independently of browser thread cache, restart with `ACP_BRIDGE=0`, reload the page, then reopen the card. Check the disabled startup log, API `sync.acp.enabled=false`, and the `off · polling` row together.
...
- DEVIN_API_KEY, ATTENTIONHQ_GITHUB_TOKEN, ATTENTIONHQ_OPENROUTER_API_KEY, DEVIN_ACP_API_KEY (optional, for the ACP bridge)
